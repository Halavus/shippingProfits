...
with open('profits.csv', mode='w', newline='') as ag:
    ...
    agWriter.writerow(['ticker', 'Volume || weight', 'Units/ship', 'Ask Katoa', 'Bid Katoa', 'Ask Prom', 'Bid Prom', 'Ask Montem', 'Bid Montem', '', 'Kat -> Prom', 'Kat -> Montem', 'Prom -> Kat', 'Prom -> Montem', 'Montem -> Kat', 'Montem -> Prom', '', 'Best route', 'Max sales/ship', 'Investment'])
    def jsonparse(ticker, label):
        try:
            output = data[ticker][label]
        except (KeyError, ValueError) as e:
            output = None
        return output

    def test(testvalue, ticker, cx):
        return jsonparse(ticker, cx+"."+testvalue)
    ...
    def get_row(ticker):
        ...
        # Write line
        agWriter.writerow([ticker, weightOrVol, math.floor(400/weightOrVol), katAsk, katBid, promAsk, promBid, montAsk, montBid,"",
    ...
    with open(fi) as f:
        ...
        for i in data.keys():
            ...
            '''
            data = json.load(f)
            agWriter.writerow(["Agricultural tickers"])
            get_row("HOP")
            get_row("CAF")
            get_row("GRN")
            get_row("MAI")
